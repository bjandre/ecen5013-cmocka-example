...
void test_complex_roots(void **state)
{
...
void test_real_roots(void **state)
{
  ...
  enum quadratic_status status = solve_quadratic(coeffs, &roots);
  assert_int_equal(status, QUAD_SUCCESS);
  assert_true(fabs(roots.root_1 - 2.0) < quadratic_epsilon);
  assert_true(fabs(roots.root_2 + 0.5) < quadratic_epsilon);
}
...
int main(int argc, char **argv)
{
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(test_complex_roots),
  };
